...
if ! gh auth status > /dev/null 2>&1; then
  echo "You are not logged in to gh CLI. Please run 'gh auth login' and try again"
  exit 1
fi

line_count=$(wf whoami | wc -l)
if [ $line_count -eq 0 ]; then
  echo "Please log into Wayfinder with 'wf login'"
  # exit 1
fi

# create a Wayfinder Workspace Access Token (WAT) for the repo
